...
- **(Group B — dataset variant.)** The curated whole-body set vs. an alternative source set,
...
examples; the A-series and the adopted policy are withheld (IP).
...
| *(pre-transfer)* | policy state before this port | not deployed | — | withheld |
...
why the controlled study mattered: it showed the knob expected to work didn't, and why.
